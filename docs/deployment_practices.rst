...
|::										|::                                                                                |
|										|                                                                                  |
|    Header set Cache-Control "max-age=0, no-cache, no-store, must-revalidate"	|    add_header Cache-Control "max-age=0, no-cache, no-store, must-revalidate";    |
|    Header edit Set-Cookie ^(.*)$ $;HttpOnly					|    add_header Pragma no-cache;                                                   |
|    Header set Pragma "no-cache"						|    add_header Expires -1;                                                        |
|    Header set Expires "-1"							|    add_header X-Frame-Options DENY;                                              |
|    Header always append X-Frame-Options: DENY					|    add_header X-XSS-Protection "1; mode=block";                                  |
|    Header set X-XSS-Protection: "1; mode=block"				|    add_header X-Content-Type-Options nosniff;                                    |
|    Header set X-Content-Type-Options: nosniff					|    add_header Content-Security-Policy "default-src 'self'";                      |
|    Header set X-Content-Security-Policy: "default-src 'self'"			|    add_header X-Download-Options: noopen;                                        |
|    Header set X-Download-Options: noopen					|    add_header X-Permitted-Cross-Domain-Policies master-only;                     |
